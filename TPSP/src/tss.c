...
tss_t tss_create_user_task(paddr_t code_start) {
  ...
  kassert(cr3 != 0, "Fallo al querer inicializar la tarea");
...
tss_t* obtenerTSS(uint16_t segsel) {
  ...
  return (tss_t*)(gdt[idx].base_31_24 << 24 | gdt[idx].base_23_16 << 16 | gdt[idx].base_15_0);
}

uint32_t obtenerCR3(uint16_t segsel) {
  return obtenerTSS(segsel)->cr3;
}

uint32_t obtenerESP0(uint16_t segsel) {
  return obtenerTSS(segsel)->esp0;
}
